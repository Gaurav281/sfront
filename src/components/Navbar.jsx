...
export default function Navbar({ activePage, onNavigate }) {
  ...
  return (
    <nav className="sticky top-0 bg-[#070707]/90 backdrop-blur-md border-b border-border-dark z-40 select-none">
          ...
          <div className="hidden md:flex items-center space-x-6">
            {navLinks.map((link) => (
              <button
                ...
                className={`relative text-xs font-semibold uppercase tracking-wider transition-colors hover:text-accent-green flex items-center gap-1 ${
            ...
            {isAdmin && (
              <button
                ...
                className={`text-xs font-bold uppercase tracking-wider transition-colors hover:text-accent-green border border-accent-green/30 bg-accent-green/5 px-2.5 py-1 rounded-lg ${
            ...
            {user ? (
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-xs font-bold text-zinc-300 hover:text-white px-3 py-2 rounded-lg transition-colors"
                >
                  Login
                </button>
                <button
                  onClick={() => onNavigate('signup')}
                  className="bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 hover:border-zinc-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-all"
                >
                  Sign Up
                </button>
              </div>
      ...
      {/* Mobile Drawer menu - Right sliding drawer with solid black background */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            {/* Sidebar menu panel - Solid background bg-black */}
            <motion.div
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              exit={{ x: '100%' }}
              transition={{ type: 'tween', ease: 'easeOut', duration: 0.25 }}
              className="relative w-full max-w-[280px] h-full bg-black border-l border-zinc-900 flex flex-col justify-between p-6 shadow-2xl z-10"
            >
              <div>
                ...
                <div className="mt-6 flex flex-col gap-2">
                  {navLinks.map((link) => (
                    <button
                      ...
                      className={`w-full text-left py-2.5 px-3 rounded-lg text-xs font-semibold tracking-wide border transition-all flex items-center justify-between ${
                        activePage === link.id
                  ...
                  {user && (
                    <button
                      ...
                      className={`w-full text-left py-2.5 px-3 rounded-lg text-xs font-semibold tracking-wide border transition-all ${
                        activePage === 'profile'
                          ? 'bg-zinc-900 text-accent-green border-zinc-800 font-bold'
                          : 'text-zinc-400 hover:bg-zinc-900/50 hover:text-white border-transparent'
                      }`}
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className={`w-full text-left py-2.5 px-3 rounded-lg text-xs font-bold tracking-wide border mt-2 transition-all ${
                        activePage === 'admin'
                ...
                {user ? (
                  <div className="flex flex-col gap-3">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-1.5 w-full bg-red-950/40 border border-red-900/50 hover:bg-red-900/40 text-red-400 font-bold py-2.5 rounded-xl transition-all text-xs"
                    >
                      <LogOut className="w-4 h-4" />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="text-center font-bold text-zinc-300 py-2 border border-zinc-900 rounded-xl text-xs hover:bg-zinc-900"
                    >
                      Login
                    </button>
                    <button
